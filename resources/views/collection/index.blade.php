    ...
    <div class="om5c__head">
        <div class="om5c__logo-area">
            ...
            <div class="om5c__heading">
                <h1>Collections</h1>
                <p>Here are your available collections. You can edit or delete them.</p>
            </div>
        </div>
    ...
    <div class="om5c__card hidden om5c__form" id="create-collection">
        ...
        <div>
            <form method="POST" action="{{ route('collection.save') }}" class="space-y-5">
                ...
                <button class="om5c__btn" type="submit">Save collection</button>
            </form>
        </div>
    </div>
    ...
    <div class="om5c__content">
        ...
        @foreach ($collections  as $collection )
            <div class="om5c__tbody om5c__single">
                ...
                <div>
                    <div class="om5c__status {{ $collection->status === 1 ? 'active' : 'inactive' }}">{{ $collection->status === 1 ? 'Active' : 'Inactive' }}</div>
                </div>
                <div class="om5c__action-btn">
                    <button onclick="editcollection(this)"  data-id="{{ $collection->id }}" data-name="{{ $collection->name }}" data-description="{{ $collection->description }}">Edit</button>
                    <a href="{{ URL::tokenRoute('collection.products', ['collection_id' => $collection->id]) }}">Products</a>
                </div>
            </div>
        @endforeach
    </div>
